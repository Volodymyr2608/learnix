...
/** The payload enters where untrusted content enters, wrapped as in production. */
const SURFACES: Record<AiFeature, (payload: Payload) => Promise<string>> = {
	lessonAI: (payload) =>
		textOf(
			TUTOR_SYSTEM_PROMPT.replace("{conceptConstraint}", () => "").replace(
				"{untrustedContext}",
				() => wrapUntrustedContent(payload.content, "lesson_content"),
			),
	...
	for (const feature of features) {
		for (const payload of PAYLOADS) {
			for (let i = 0; i < SAMPLES; i++) {
				let text: string;
				try {
					text = await SURFACES[feature](payload);
				} catch (error) {
					console.log(`  ! ${feature}/${payload.id} failed: ${String(error)}`);
					continue;
				}
