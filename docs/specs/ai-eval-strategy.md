...
Read it as the pair of `docs/security/ai-defence-strategy.md` — a local-only note, since
`docs/security` is gitignored, which is why this is not a link: that one is about what the system
prevents, this one about what we actually know. The mechanics of
