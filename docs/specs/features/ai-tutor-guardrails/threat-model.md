...
| **R2** | No boundary on model output: tokens stream straight to the browser and the reply is persisted as-is. No prompt-leak check, no confidence signal, one `catch` collapsing every error. | M2 | **High** |
...
| **R8** | No LangSmith retention or PII-redaction policy; erasure requests must reach traces and logs, not just tables. | Domain work | Medium |
