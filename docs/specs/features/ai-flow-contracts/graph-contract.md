...
|---|---|---|---|---|---|---|
| `classify_intent` | classifies the turn as continue / revise / clarify, and resolves the revise target from the field the model names; a target resolving to the step being collected is a continue | `history`, `userMessage`, `currentStep`, `content`, `instructorId`, `generationId` | `intent`, `reviseTarget` | `routeByIntent` | gpt-4o-mini, structured | swallowed locally → falls back to `continue`, **emitting `fallback_triggered`** |
...
| `chat_response` | streams the reply across four prompt branches | `userMessage`, `currentStep`, `content`, `intent`, `history` | `assistantText` | `output_boundary` | gpt-4o-mini, streaming | propagates; a mid-stream drop loses the partial reply |
| `output_boundary` | runs the shared output boundary over the assembled reply, silently | `assistantText` | `outputRejected` | `routeAfterOutputBoundary` | none | cannot fail — a validator exception is itself a rejection |
...
| `clarify` | streams one clarifying question | `validationErrors`, `assessClarify`, `currentStep`, `draftStepData`, last 4 `history`, `userMessage` | `assistantText` | `output_boundary_clarify` | gpt-4o-mini, streaming | propagates |
| `output_boundary_clarify` | the same node, registered again after `clarify` | `assistantText` | `outputRejected` | END | none | cannot fail |
...
| `routeAfterOutputBoundary` | `outputRejected` | `rejected` → END; `assess` → `assess_completion` |
